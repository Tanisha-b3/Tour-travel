        ...
        {images.length > 1 && (
          <>
            <motion.button
              ...
              whileTap={{ scale: 0.9 }}
              className="absolute top-1/2 -translate-y-1/2 left-4 z-20 w-11 h-11 rounded-full bg-black/50 backdrop-blur-sm text-white flex items-center justify-center text-2xl cursor-pointer hover:bg-[#0ea5e9]/80 transition-colors opacity-0 group-hover:opacity-100"
            >
              ‹
            </motion.button>
            <motion.button
              ...
              whileHover={{ scale: 1.1 }}
              whileTap={{ scale: 0.9 }}
              className="absolute top-1/2 -translate-y-1/2 right-4 z-20 w-11 h-11 rounded-full bg-black/50 backdrop-blur-sm text-white flex items-center justify-center text-2xl cursor-pointer hover:bg-[#0ea5e9]/80 transition-colors opacity-0 group-hover:opacity-100"
            >
              ›
            </motion.button>
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-20">
              {images.map((_, i) => (
                <motion.button
                  ...
                  animate={{ width: i === current ? 24 : 8, backgroundColor: i === current ? "#0ea5e9" : "rgba(255,255,255,0.5)" }}
        ...
        <div className="flex gap-2.5 mt-3 justify-center">
          {images.map((img, i) => (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              animate={{ opacity: i === current ? 1 : 0.45, borderColor: i === current ? "#0ea5e9" : "transparent" }}
